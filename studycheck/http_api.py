...
def daily_queue(service:StudyCheckService,user_id:str,limit:int=20)->list[dict[str,Any]]:
...
def review(service:StudyCheckService,payload:dict[str,Any])->dict[str,Any]:
    if not isinstance(payload,dict) or not payload.get('user_id') or not payload.get('knowledge_point') or not isinstance(payload.get('correct'),bool):
        raise APIError(400,'invalid_review','user_id, knowledge_point and boolean correct are required')
